...
public class ClassRoster
{
   ...
   public String studentWithMaxGPA()
      ...
      {
          if(studentList.get(index).calcGPA() > studentList.get(outputIndex).calcGPA())
          {
              outputIndex = index;
          }
      }
      
      return studentList.get(outputIndex).toString();
   ...
   public int dropStudent( int minGradeLevel )
      ...
      for(int index = 0; index < studentList.size(); index++)
      {
          if(studentList.get(index).getGradeLevel() < (double) minGradeLevel)
          {
